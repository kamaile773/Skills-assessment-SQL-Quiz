...
SELECT cupcakes.name, SUM(orders.num_cupcakes) As sum
 FROM cupcakes FULL OUTER JOIN orders ON orders.cupcake_id = cupcakes.id
...
SELECT customers.email FROM customers, orders 
WHERE orders.customer_id = customers.id 
GROUP BY orders.customer_id, customers.email 
...
SELECT customers.fname, customers.lname,customers.email from customers,orders 
WHERE customers.id = orders.customer_id AND orders.cupcake_id in (SELECT id from cupcakes where name = 'funfetti') AND orders.processed = True
